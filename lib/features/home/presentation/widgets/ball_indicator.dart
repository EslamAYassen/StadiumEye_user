...
class ShakeState {
  ...
  ShakeState({required this.shift, required this.isInitial});
}
...
class BallIndicator extends StatefulWidget {
  ...
  const BallIndicator({
    ...
    this.ballColors = const [Colors.white],
    this.controller,
...
class _BallIndicatorState extends State<BallIndicator>
  ...
  @override
  void initState() {
    _ballColor = widget.ballColors.length > 1
        ? widget.ballColors.random
        : Colors.white;
  ...
  void _onHitBorder(Offset direction) {
    setState(() {
      ...
      if (widget.ballColors.length > 1) {
        final colors = widget.ballColors
            .where((color) => color != _ballColor)
            .toList();
  ...
  void _onTick(Duration time) {
    ...
    final Size ballSafeSpace = Size(
      _rectSize.width - widget.ballRadius * 2,
  ...
  double _calculateAngle({required Offset origin, required Offset point}) {
    final double angle =
  ...
  final _centerTween = Tween<Offset>(begin: Offset.zero, end: Offset.zero);
            ...
            final ball = AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                border: controller.isArmed
                    ? Border.all(color: Colors.white, width: widget.strokeWidth)
                    : null,
              ),
              width: ballSize,
              height: ballSize,
